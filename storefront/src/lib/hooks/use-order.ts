...
export function useOrder(id: string) {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    setState("loading")
    try {
      const resp = await fetch(`${BACKEND_URL}/store/orders/${id}?fields=*items,+items.thumbnail,+shipping_address,+metadata,*fulfillments,*fulfillments.labels,*shipping_methods`, {
        headers: {
          ...getMedusaHeaders(),
        },
        credentials: "include",
