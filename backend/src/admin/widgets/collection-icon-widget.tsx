...
const CollectionIconWidget = ({ data }: WidgetProps) => {
  ...
  const uploadAndSave = async () => {
    ...
    try {
      ...
      if (!saveRes.ok) {
        throw new Error("Failed to save collection metadata")
      }
  ...
  return (
    ...
    >
      <div style={{ padding: "16px", borderBottom: "1px solid #e5e7eb" }}>
        <h2 style={{ margin: 0, fontSize: "14px", fontWeight: 600 }}>Иконка коллекции</h2>
      </div>

      <div style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "10px" }}>
        <p style={{ margin: 0, fontSize: "12px", color: "#6b7280" }}>
          Коллекция: <b>{data.title || data.id}</b>
        </p>
